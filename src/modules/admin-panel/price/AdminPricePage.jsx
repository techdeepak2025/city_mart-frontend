...
import { productPricingSchema } from "./validation";
...
export default function AdminPricePage() {
  ...
  const handleDiscountChange = (index, newDiscount) => {
    ...
    const price = isNaN(mrp) ? 0 : Math.round(mrp - (discount / 100) * mrp);

    product.discount = discount;
    product.price = price;

    const result = productPricingSchema.safeParse({ mrp, discount, price });
  ...
  const savePrice = async (productId, price, discount) => {
    if (errors[productId]) {
    ...
    try {
      await axiosInstance.patch(`/products/${productId}`, { price, discount });
        ...
        <AnimatePresence>
          {products.map((product, index) => {
            ...
            const displayPrice =
              product.price ??
              Math.round(
                (product.mrp ?? 0) - ((product.discount ?? 0) / 100) * (product.mrp ?? 0)
              );
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-gray-600">
                  MRP: ₹{isNaN(product.mrp) ? "N/A" : product.mrp}
                </p>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <NumberInput
                    ...
                    onChange={(val) => {
                      ...
                      handleDiscountChange(index, isNaN(parsed) ? 0 : parsed);
                    }}
                  />
                  {errors[product._id] && (
                ...
                <div className="flex justify-end gap-2 mt-auto pt-2">
                  ...
                  <SaveButton
                    onClick={() =>
                      savePrice(product._id, displayPrice, product.discount)
                    }
